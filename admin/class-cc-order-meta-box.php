...
class CC_Order_Meta_Box {
    ...
    public function ajax_create_voucher() {
        ...
        if ( $boxnow ) {
            ...
            if ( stripos( $contractor_note, 'No locker found' ) !== false ) {
                ...
                // Πιθανά fields που επιστρέφει το Courier Center API με το τελικό locker
                ...
                // Fallback: προσπαθούμε να εξάγουμε locker code από το ContractorResultNote
    ...
    private function get_requested_locker_code( $order ) {
        if ( $order->get_meta( '_boxnow_delivery_mode' ) === 'pick' ) {
            return $order->get_meta( '_boxnow_locker_code' );
        }
        return '';
    }
